//Line 238: I set the listened value to No instead of false because it looks
//better, but if it doesn't work later then change it back to false
//I also changed the Rating value to N/A instead of 0 so if it doesn't work
//then change it also
...
let ItemFeedbackDB = require('./../utility/ItemFeedbackDB');
...
router.all('/profile', (request, response, next) => {
  ...
  if (typeof sessionProfile != 'undefined') { //session data exists. Use that.
    ...
    let theUser = UserDB.getUser("cking107@uncc.edu");
...
router.post('/profile', function (request, response) {
  ...
  if (action == null || action == "" || action == "showProfile") {
    ...
    respData = updateProfileListened(request, response);
...
let addItem = function (request) {
  ...
  let sessionProfile = request.session.currentProfile;
  let userId = request.session.theUser.userID;
  ...
  if (ItemDB.exists(itemCode)) {
    ...
    profileItem.setRating("N/A");
    profileItem.setListened("No");
...
let updateProfileRate = function (request, response) {
  ...
  if ((isItemInView(viewItems, itemCodeParam)) && (isItemInProfile(userProfile, itemCodeParam))) {
    ...
    if (typeof ratingParam != 'undefined' && ratingParam <= 10 && ratingParam >= 1) {
...
let updateProfileListened = function (request, response) {
  let userProfile = request.session.currentProfile;
  ...
  if ((isItemInView(viewItems, itemCodeParam)) && (isItemInProfile(userProfile, itemCodeParam))) {
    ...
    if (typeof listenedParam != 'undefined') {
      ...
      //console.log(userItem);
      userProfileObj.updateItemFlag(userItem, listenedParam);
      request.session.currentProfile = userProfileObj.getItems();
      //console.log("profile from session")
      //console.log(request.session.currentProfile);
    } else {
